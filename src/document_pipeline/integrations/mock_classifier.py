    ...
    async def classify(self, value: ClassificationInput) -> ClassificationResult:
        ...
        if value.nlp_type == "ORG" or re.search(
            ...
            confidence = (
                0.96
                if re.search(
                    r"\b(works at|announced|acquired|subsidiary|headquartered)\b", context, re.I
                )
                else 0.9
            )
            return self._result(
                ...
                confidence,
                "The token contains an organization suffix or company context.",
            )
        if value.nlp_type == "PERSON" or re.fullmatch(
            r"(?:Dr\.|Ms\.|Mr\.)?\s*[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2}", text
        ):
            return self._result(
                Classification.PERSON, 0.94, "The token matches a full personal name."
            )
        if value.nlp_type == "ADDRESS" or re.search(
            r"\b\d{1,6}\s+.+\s+(St|Street|Ave|Road|Rd|Blvd|Lane|Drive|Dr)\b", text
        ):
            return self._result(
                Classification.ADDRESS, 0.95, "The token follows a street-address pattern."
            )
        if value.nlp_type == "DATE" or re.search(
            r"\d{4}-\d{2}-\d{2}|\d{1,2}/\d{1,2}/\d{2,4}", text
        ):
            return self._result(
                Classification.DATE, 0.93, "The token is formatted as a calendar date."
            )
        return self._result(
            Classification.UNKNOWN,
            0.55,
            "The token is not specific enough for a supported category.",
        )

    def _result(
        self, classification: Classification, confidence: float, reasoning: str
